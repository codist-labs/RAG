...
# ── 3b. Airflow 2.8.1's ORM models use legacy pre-2.0 annotations that
# SQLAlchemy 2.x's stricter Declarative system rejects with
# MappedAnnotationError (e.g. on TaskInstance.dag_model). Confirmed as
# a known Airflow/SQLAlchemy-2.0 migration issue upstream. Fix: set
# __allow_unmapped__ = True on Airflow's shared declarative Base before
# anything imports airflow.models (which defines mapped classes against
# it). Defensive here in case any test module imports airflow.models
# directly — tests/test_dag_integrity.py applies this same patch itself
# too, so this is redundant for that file specifically but harmless.
try:
