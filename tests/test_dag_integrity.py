...
FIX 4: Airflow 2.8.1's ORM models (e.g. TaskInstance) use legacy,
pre-SQLAlchemy-2.0 type annotations that SQLAlchemy 2.x's stricter
Declarative system rejects by default with:
  sqlalchemy.orm.exc.MappedAnnotationError: Type annotation for
  "TaskInstance.dag_model" can't be correctly interpreted for
  Annotated Declarative Table form.
Confirmed as a known, actively-being-fixed incompatibility in
Airflow's own upstream "SQLAlchemy 2.0 deprecations" migration effort
(hits this identical error). The fix is SQLAlchemy's own documented
escape hatch: set `Base.__allow_unmapped__ = True` on Airflow's shared
declarative Base before importing anything that defines a mapped class
against it — see section 2b below.
"""
...
# ── 2b. Airflow 2.8.1's ORM models (e.g. TaskInstance) use legacy,
# pre-2.0 explicit type annotations that SQLAlchemy 2.x's stricter
# "Annotated Declarative Table" system rejects by default with:
#   sqlalchemy.orm.exc.MappedAnnotationError: Type annotation for
#   "TaskInstance.dag_model" can't be correctly interpreted for
#   Annotated Declarative Table form. ... To allow Annotated Declarative
#   to disregard legacy annotations which don't use Mapped[] to pass,
#   set "__allow_unmapped__ = True" on the class or a superclass.
# This is SQLAlchemy's own officially documented escape hatch for
# exactly this situation (confirmed against Airflow's own upstream
# "Fix all deprecations for SQLAlchemy 2.0" migration effort, which
# hits this identical error). We set it on Airflow's shared declarative
# Base BEFORE importing anything that defines a mapped class against
# it (airflow.models.taskinstance.TaskInstance, imported transitively
# by `from airflow.models import DAG, DagBag` below) — Python resolves
# __allow_unmapped__ via normal attribute inheritance, so setting it on
# the shared Base is sufficient for every subclass, including ones
# defined later during this same import chain.
try:
    from airflow.models.base import Base as _AirflowBase
    _AirflowBase.__allow_unmapped__ = True
except Exception:
    pass
